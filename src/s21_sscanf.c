...
int s21_sscanf(const char *str, const char *format, ...) {
  ...
  if (str == s21_NULL || format == s21_NULL || (int)s21_strlen(str) == 0 ||
      (int)s21_strlen(format) == 0) {
    is_continue = 1;
    result = -1;
  }
...
char *s21_string_handler(char *str, char *string, char *buffer_string,
  ...
  if (flag == 0) {
    switch (specifier) {
      case 'd': {
        char *new_string_pointer = s21_d_handler(
            string, args, width, length, is_ignoring, is_continue, result);
        string = new_string_pointer;

        break;
      }
      case 'i': {
        char *new_string_pointer = s21_i_handler(
            string, args, width, length, is_ignoring, is_continue, result);
        string = new_string_pointer;
      ...
      case 'u': {
        char *new_string_pointer = s21_u_handler(
            string, args, width, length, is_ignoring, is_continue, result);
        string = new_string_pointer;
      ...
      case 'o': {
        char *new_string_pointer = s21_o_handler(
            string, args, width, length, is_ignoring, is_continue, result);
        string = new_string_pointer;
      ...
      case 'X': {
        char *new_string_pointer =
            s21_x_handler(string, args, width, length, specifier, is_ignoring,
                          is_continue, result);
        string = new_string_pointer;
      ...
      case 'G':
      case 'f': {
        char *new_string_pointer = s21_f_handler(
            string, args, width, length, is_ignoring, is_continue, result);
        string = new_string_pointer;
      ...
      case 'c': {
        char *new_string_pointer =
            s21_c_handler(string, args, length, is_ignoring);
        string = new_string_pointer;
      ...
      case 's': {
        char *new_string_pointer =
            s21_s_handler(string, args, width, length, is_ignoring);
        string = new_string_pointer;

        break;
      }
      case 'n': {
        char *new_string_pointer = s21_n_handler(string, str, buffer_string,
                                                 args, length, is_ignoring);
        string = new_string_pointer;
...
char *s21_string_to_octal(char *string, unsigned long long int *variable,
  ...
  *variable = 0;
  ...
  if (width > 0) {
    while (is_octal_digit(*string) && temp_counter < width - is_width_cutted) {
...
char *s21_string_to_hex(char *string, unsigned long long int *variable,
  ...
  *variable = 0;
  ...
  if (width > 0) {
    while (is_hex_digit(*string) && temp_counter < width - is_width_cutted) {
      temp_string[temp_counter] = *string;
      temp_counter++;
      string++;
    }
...
char *s21_d_handler(char *string, va_list args, int width, char length,
...
char *s21_o_handler(char *string, va_list args, int width, char length,
                    int is_ignoring, int *is_continue, int *result) {
  ...
  if (is_octal_digit(*string)) {
    ...
    char *new_string_pointer =
        s21_string_to_octal(string, variable_pointer, width, is_width_cutted);
    string = new_string_pointer;
...
char *s21_x_handler(char *string, va_list args, int width, char length,
                    char specifier, int is_ignoring, int *is_continue,
                    int *result) {
  // Убирем лишние пробелы в строке
  int begin_space_number = (int)s21_strspn(string, " ");
  string += begin_space_number;
  ...
  if (is_hex_digit(*string)) {
    ...
    char *new_string_pointer =
        s21_string_to_hex(string, variable_pointer, width, is_width_cutted);
    string = new_string_pointer;
...
char *s21_u_handler(char *string, va_list args, int width, char length,
                    int is_ignoring, int *is_continue, int *result) {
  // Убирем лишние пробелы в строке
  int begin_space_number = (int)s21_strspn(string, " ");
  string += begin_space_number;
...
char *s21_i_handler(char *string, va_list args, int width, char length,
                    int is_ignoring, int *is_continue, int *result) {
  // Убирем лишние пробелы в строке
  int begin_space_number = (int)s21_strspn(string, " ");
  string += begin_space_number;
...
char *s21_f_handler(char *string, va_list args, int width, char length,
                    int is_ignoring, int *is_continue, int *result) {
  // Убираем лишние пробелы в строке
  int begin_space_number = (int)s21_strspn(string, " ");
  string += begin_space_number;
    ...
    if (*string == '.' &&
      ...
      if (width > 0) {
        if (is_digit(*string) && width_counter < width - is_width_cutted) {
          ...
          while (is_digit(*string) && width_counter < width - is_width_cutted) {
            number_after_dot = *string - '0' + number_after_dot * 10.0;
            string++;
            number_length_counter++;
            width_counter++;
          }
...
char *s21_s_handler(char *string, va_list args, int width, char length,
                    int is_ignoring) {
  int begin_space_number = (int)s21_strspn(string, " ");
  string += begin_space_number;
...
void write_to_variable(va_list args, unsigned long long int *variable_pointer,
                       char length) {
  if (length == 'h') {
...
void write_to_unsigned_variable(va_list args,
                                unsigned long long int *variable_pointer,
                                char length) {
  if (length == 'h') {
    unsigned short int *variable = va_arg(args, unsigned short int *);
    *variable = (unsigned short int)*variable_pointer;
  } else if (length == 'l') {
    unsigned long int *variable = va_arg(args, unsigned long int *);
