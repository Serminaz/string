...
int s21_sprintf(char *str, const char *format, ...) {
  ...
  while (*format && error == 0) {
    ...
    if (flag == 1 && is_len(*format)) FFormat.len = *(format++);

    if (flag == 1 && is_spec(*format)) FFormat.spec = *(format++);
...
char *s21_format_param_to_str(char *str, s21_size_t *str_len, FORMAT *FFormat,
  ...
  if (temp_str != s21_NULL) {
    switch (FFormat->spec) {
      case 'd':
      case 'i':
        s21_str_from_d(temp_str, argList, FFormat);
        break;
      case 'o':
        s21_str_from_o(temp_str, argList, FFormat);
        break;
      case 'u':
        s21_str_from_u(temp_str, argList, FFormat);
        break;
      case 'x':
      case 'X':
        s21_str_from_xX(temp_str, argList, FFormat);
        break;
      case 'e':
      case 'E':
        s21_str_from_eE(temp_str, argList, FFormat);
        break;
      case 'f':
        s21_str_from_f(temp_str, argList, FFormat);
        break;
      case 'g':
      case 'G':
        s21_str_from_gG(temp_str, argList, FFormat);
        break;
      case 'c':
        s21_str_from_c(temp_str, argList, FFormat);
        break;
      case 's':
        s21_str_from_s(temp_str, argList, FFormat);
        break;
      case 'p':
        s21_str_from_p(temp_str, argList, FFormat);
        break;
      case 'n':
        s21_str_from_n(str_len, argList);
        break;
      case '%':
        s21_str_from_perc(temp_str, FFormat);
        break;
    }
